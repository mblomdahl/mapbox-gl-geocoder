 ...
 * @param {HTMLInputElement} [options.inputEl] Custom input element to attach geocoder to.
 * @param {HTMLElement} [options.clearEl] Optional HTML element for clearing the search box, only used in conjunction with an `inputEl`.
 * @example
...
function MapboxGeocoder(options) {
  ...
  if (this.options.inputEl) {
    ...
    if (this.options.clearEl) {
      this._clearEl = this.options.clearEl;
...
MapboxGeocoder.prototype = {
  ...
  _onKeyDown: debounce(function(e) {
    if (!e.target.value) {
      return this._clearEl ? this._clearEl.style.display = 'none' : null;
    }
  ...
  _onChange: function() {
    if (this._inputEl.value && this._clearEl) this._clearEl.style.display = 'block';
    var selected = this._typeahead.selected;
    if (selected) {
      if (this._map && this.options.flyTo) {
  ...
  _geocode: function(searchInput) {
    ...
    request.then(function (response) {
      ...
      if (res.features.length) {
        if (this._clearEl) this._clearEl.style.display = 'block';
      } else {
